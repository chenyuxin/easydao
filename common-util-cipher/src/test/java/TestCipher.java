...
public class TestCipher {
	...
	private static byte[] WDSM4key = new byte[]{17, 34, 79, 88, -120, 16, 64, 56, 40, 37, 121, 81, -53, -35, 85, 102};
	...
	public void testSM4(){
		...
		try {
			byte[] oStr = Cipher.WDSM4.decrypt(HexUtil.hexStringToBytes(enString),WDSM4key);
	...
	public void testSM4e(){
		...
		try {
			long lStart = System.currentTimeMillis();
			byte[] md = Cipher.WDSM4.encrypt(xml.getBytes(),WDSM4key);
			//String cipher = BASE64.encryptBASE64(md);
			String cipher = HexUtil.toHexString(md);
			System.out.println(cipher);
	        long lUseTime = System.currentTimeMillis() - lStart;  
		    System.out.println("加密耗时：" + lUseTime + "毫秒");  
		    byte[] oStr = Cipher.WDSM4.decrypt(md,WDSM4key);
			System.out.println(new String(oStr,CharsetUtil.CHARSET_UTF_8));
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		try {
			long lStart = System.currentTimeMillis();
			byte[] md = Cipher.SM4.encrypt(xml.getBytes(),WDSM4key);
			//String cipher = BASE64.encryptBASE64(md);
			String cipher = HexUtil.toHexString(md);
			System.out.println(cipher);
	        long lUseTime = System.currentTimeMillis() - lStart;  
		    System.out.println("加密耗时：" + lUseTime + "毫秒");  
		    byte[] oStr = Cipher.SM4.decrypt(md,WDSM4key);
			System.out.println(new String(oStr,CharsetUtil.CHARSET_UTF_8));
		} catch (Exception e) {
			e.printStackTrace();
		}
