...
import com.wondersgroup.commonutil.customcipher.WdSM4;
...
//	},
	 ...
	 * 万达信息SM4 无向量
	 */
	WDSM4(CipherType.Custom,WdSM4.class.getName()) {
		@Override
	...
	public String encrypt(String source,Object... confs) throws Exception {
		...
		for (Object conf : confs) {
			if (conf instanceof Charset) {
				charset = (Charset) conf;
			} else if (conf instanceof ByteConverter) {
				byteConverter = (ByteConverter) conf;
			}
		}
	...
	public String encrypt(byte[] source,byte[] key,Object... confs) throws Exception {
		...
		for (Object conf : confs) {
			if (conf instanceof ByteConverter) {
				byteConverter = (ByteConverter) conf;
			} else if (conf instanceof String) {
				ivString = (String) conf;
